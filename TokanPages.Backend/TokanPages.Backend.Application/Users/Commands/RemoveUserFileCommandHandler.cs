...
public class RemoveUserFileCommandHandler : RequestHandler<RemoveUserFileCommand, Unit>
{
    ...
    public RemoveUserFileCommandHandler(ILoggerService loggerService, IUserService userService, 
        IAzureBlobStorageFactory azureBlobStorageFactory) : base(loggerService)
    {
    ...
    public override async Task<Unit> Handle(RemoveUserFileCommand request, CancellationToken cancellationToken)
    {
        ...
        {
            UserFileToUpdate.Image => "images",
            UserFileToUpdate.Audio => "audio",
            UserFileToUpdate.Video => "videos",
            UserFileToUpdate.Document => "documents",
            UserFileToUpdate.Application => "applications",
