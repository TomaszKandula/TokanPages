...
export const UpdateComponents = (state: ContentPageDataState, source: ContentModelDto[]): ComponentsDto => {
    ...
    source.forEach(item => {
        ...
        result.activateAccount = item.contentName === "activateAccount" ? item as ActivateAccountContentDto : result.activateAccount;
        result.article = item.contentName === "article" ? item as ArticleContentDto : result.article;
        result.articleFeatures = item.contentName === "articleFeatures" ? item as ArticleFeaturesContentDto : result.articleFeatures;
        result.businessForm = item.contentName === "businessForm" ? item as BusinessFormContentDto : result.businessForm;
        ...
        result.cookiesPrompt = item.contentName === "cookiesPrompt" ? item as CookiesPromptContentDto : result.cookiesPrompt;
        ...
        result.newsletterRemove = item.contentName === "newsletterRemove" ? item as NewsletterRemoveContentDto : result.newsletterRemove;
        result.newsletterUpdate = item.contentName === "newsletterUpdate" ? item as NewsletterUpdateContentDto : result.newsletterUpdate;
        result.resetPassword = item.contentName === "resetPassword" ? item as ResetPasswordContentDto : result.resetPassword;
        result.technologies = item.contentName === "technologies" ? item as TechnologiesContentDto : result.technologies;
        result.templates = item.contentName === "templates" ? item as TemplatesContent : result.templates;
        result.testimonials = item.contentName === "testimonials" ? item as TestimonialsContentDto : result.testimonials;
        result.updatePassword = item.contentName === "updatePassword" ? item as UpdatePasswordContentDto : result.updatePassword;
